...
describe('Reporting — dashboard e relatórios (integração)', () => {
  ...
  it('DRE gerencial bate com o esperado (receita líquida - CMV - despesas)', async () => {
    ...
    expect(dre.body.profitDistribution).toMatchObject({
      applied: true,
      baseAmount: '100',
      reinvestment: { rate: '60', amount: '60' },
      proLabore: { rate: '25', amount: '25' },
    });
  });
  ...
  it('aplica a política de distribuição válida no período sem transformá-la em despesa', async () => {
    const { session } = await setupSaleAndExpense();
    await request(app.getHttpServer())
      .post('/api/v1/company/profit-distribution')
      .set(auth(session.accessToken))
      .send({
    ...
    expect(dre.body.netProfit).toBe('100');
    expect(dre.body.expenses).toBe('50');
    expect(dre.body.profitDistribution.reinvestment).toEqual({
      rate: '50',
      amount: '50',
    });
    expect(dre.body.profitDistribution.proLabore).toEqual({
      rate: '30',
      amount: '30',
    });
  });
