...
describe('Inventory + Purchasing — fluxo completo (integração)', () => {
  ...
  it('recebimento a prazo gera parcelas e o estorno reverte estoque e compromissos', async () => {
    ...
    const supplier = await request(app.getHttpServer())
      ...
      .set(auth(session.accessToken))
      .send({ name: 'Fornecedor parcelado' })
      .expect(201);
    const purchase = await request(app.getHttpServer())
      .post('/api/v1/purchasing/purchases')
      .set(auth(session.accessToken))
    ...
    const received = await request(app.getHttpServer())
      ...
      .send({
        items: [
          { purchaseItemId: purchase.body.items[0].id, quantityReceived: 2 },
        ],
        installmentPlan: { count: 3, firstDueDate: '2026-09-15' },
      })
      .expect(201);
    expect(received.body.payables).toHaveLength(3);
    const reversed = await request(app.getHttpServer())
      .post(`/api/v1/purchasing/purchases/${purchase.body.id}/reverse`)
      .set(auth(session.accessToken))
      .send({ reason: 'Devolução ao fornecedor' })
      .expect(201);
    expect(reversed.body.status).toBe('cancelled');
    expect(
      reversed.body.payables.every(
  ...
  it('inventário físico transforma divergência em ajuste rastreável', async () => {
    ...
    const count = await request(app.getHttpServer())
      ...
      .set(auth(session.accessToken))
      .send({ notes: 'Contagem mensal' })
      .expect(201);
    expect(count.body.items[0].expectedQuantity).toBe('5');
    await request(app.getHttpServer())
      .patch(`/api/v1/inventory/counts/${count.body.id}`)
      .set(auth(session.accessToken))
      .send({ items: [{ itemId: count.body.items[0].id, countedQuantity: 3 }] })
      .expect(200);
    await request(app.getHttpServer())
      .post(`/api/v1/inventory/counts/${count.body.id}/complete`)
      .set(auth(session.accessToken))
      .expect(201);
    const balances = await request(app.getHttpServer())
      .get('/api/v1/inventory/balances')
