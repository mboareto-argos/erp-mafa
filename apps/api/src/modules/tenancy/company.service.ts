...
export class CompanyService {
  ...
  async update(tenant: CurrentTenantContext, dto: UpdateCompanyDto) {
    const company = await this.prisma.company.findFirst({
      where: { id: tenant.companyId, deletedAt: null },
    });
    if (!company)
      throw new AppError(
        'COMPANY_NOT_FOUND',
    ...
    if (company.currency !== dto.currency) {
      ...
      if (operations.some((count) => count > 0))
        throw new AppError(
          ...
          'A moeda principal não pode ser alterada depois do início das operações.',
          HttpStatus.CONFLICT,
          'currency',
        );
    }
    ...
    const data = {
      ...
      operationStartDate: dto.operationStartDate
        ? new Date(`${dto.operationStartDate}T00:00:00.000Z`)
        : null,
      ...
      defaultMinStock: dto.defaultMinStock
        ? new Prisma.Decimal(dto.defaultMinStock.replace(',', '.'))
        : null,
      discountLimit: dto.discountLimit
        ? new Prisma.Decimal(dto.discountLimit.replace(',', '.'))
        : null,
    };
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await this.audit.record(tx, {
        ...
        beforeData: this.auditSnapshot(company),
  ...
  listProfitDistribution(companyId: string) {
    return this.prisma.profitDistributionPolicy.findMany({
    ...
    return this.prisma.$transaction(async (tx) => {
      const policy = await tx.profitDistributionPolicy.upsert({
        ...
        create: {
          ...
          proLaboreRate: dto.proLaboreRate,
          reserveRate: dto.reserveRate,
          marketingRate: dto.marketingRate,
      ...
      await this.audit.record(tx, {
        companyId: tenant.companyId,
        userId: tenant.userId,
        action: 'company.profit_distribution_updated',
        entityType: 'profit_distribution_policy',
        entityId: policy.id,
        afterData: {
          effectiveFrom: policy.effectiveFrom,
          reinvestmentRate: policy.reinvestmentRate.toString(),
          proLaboreRate: policy.proLaboreRate.toString(),
          reserveRate: policy.reserveRate.toString(),
          marketingRate: policy.marketingRate.toString(),
        },
      });
      return policy;
    });
  ...
  private auditSnapshot(company: {
    ...
    defaultMinStock: Prisma.Decimal | null;
    discountLimit: Prisma.Decimal | null;
  }) {
    return {
      name: company.name,
      ...
      allocationMethod: company.allocationMethod,
      defaultMinStock: company.defaultMinStock?.toString() ?? null,
      discountLimit: company.discountLimit?.toString() ?? null,
    };
  }
}
