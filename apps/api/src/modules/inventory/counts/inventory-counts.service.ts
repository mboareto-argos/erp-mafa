...
const include = {
  items: {
    ...
    include: {
      productVariant: { include: { product: true, stockBalances: true } },
    },
    orderBy: { productVariant: { product: { name: 'asc' as const } } },
  },
};
...
export class InventoryCountsService {
  ...
  async create(tenant: CurrentTenantContext, dto: CreateInventoryCountDto) {
    ...
    if (!variants.length)
      throw new AppError(
        'INVENTORY_COUNT_EMPTY',
        'Cadastre produtos antes de iniciar um inventário.',
        HttpStatus.CONFLICT,
      );
    return this.prisma.$transaction(async (tx) => {
        ...
        data: {
          ...
          items: {
            create: variants.map((variant) => ({
              ...
              expectedQuantity:
                variant.stockBalances[0]?.quantityAvailable ?? 0,
              createdBy: tenant.userId,
            })),
          },
        },
      ...
      await this.audit.record(tx, {
        ...
        entityId: count.id,
        afterData: { itemCount: variants.length },
      });
      return count;
    });
  }
  async update(
    ...
    const count = await this.get(tenant.companyId, id);
    if (count.status !== 'draft')
      throw new AppError(
        'INVENTORY_COUNT_CLOSED',
        'Este inventário já foi concluído.',
        HttpStatus.CONFLICT,
      );
    const valid = new Set(count.items.map((item) => item.id));
    if (dto.items.some((item) => !valid.has(item.itemId)))
      throw new AppError(
        'INVALID_INVENTORY_ITEM',
        'Um ou mais itens não pertencem ao inventário.',
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      return tx.inventoryCount.findUniqueOrThrow({ where: { id }, include });
    });
  }
  async complete(tenant: CurrentTenantContext, id: string) {
    ...
    if (count.items.some((item) => item.countedQuantity === null))
      throw new AppError(
        ...
        'Conte todos os produtos antes de concluir.',
        HttpStatus.CONFLICT,
      );
    return this.prisma.$transaction(async (tx) => {
      for (const item of count.items) {
        ...
        const current = new Prisma.Decimal(balance?.quantityAvailable ?? 0);
        const delta = new Prisma.Decimal(item.countedQuantity!).sub(current);
        ...
        await tx.stockBalance.upsert({
          ...
          update: { quantityAvailable: item.countedQuantity! },
        });
      }
      ...
      await this.audit.record(tx, {
        ...
        entityId: id,
        afterData: {
          adjustedItems: count.items.filter(
            (item) =>
              !new Prisma.Decimal(item.expectedQuantity).equals(
