...
export function QuickSaleForm({ products, methods, customers, initialOpen = false, editingSale }: { products: Product[]; methods: Method[]; customers: Customer[]; initialOpen?: boolean; editingSale?: EditableSale }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      close(); setLines([{ productId: "", quantity: "1", price: "" }]); setCustomerId(""); setMethodId(""); setDiscount(""); setIdempotencyKey(""); router.refresh();
      ...
      <div className="wizard-layout">
        <div className="wizard-main">
          ...
          {step === 1 && <>
            ...
            <fieldset className="payment-options"><legend>Forma de pagamento</legend><div>{methods.filter(method => method.status === "active").map(method => <label className={`payment-option${methodId === method.id ? " selected" : ""}`} key={method.id}><input type="radio" name="payment-method" checked={methodId === method.id} onChange={() => setMethodId(method.id)} /><span className="payment-radio" aria-hidden="true" /><span><strong>{method.name}</strong><small>{methodId === method.id ? "Selecionada para esta venda" : "Selecionar para continuar"}</small></span></label>)}</div></fieldset>
