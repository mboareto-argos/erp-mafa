...
function WizardSteps({ current }: { current: number }) {
  return (
    <ol className="wizard-steps" aria-label="Etapas da venda">
      {['Itens', 'Pagamento', 'Confirmar'].map((label, index) => (
        <li
          key={label}
          className={
...
export function QuickSaleForm({
  ...
  const [customerId, setCustomerId] = useState(editingSale?.customerId ?? '');
  const [channel, setChannel] = useState(editingSale?.channel ?? 'presencial');
  const [methodId, setMethodId] = useState('');
  const [discount, setDiscount] = useState(editingSale?.discount ?? '');
  ...
  function chooseProduct(index: number, productId: string) {
    ...
    if (product?.prices[0])
      updateLine(index, 'price', product.prices[0].salePrice);
  }
  function nextStep() {
    setError(undefined);
    if (
      ...
      return setError(
        'Complete os itens, as quantidades e os preços antes de continuar.',
      );
    if (step === 1 && paymentMode === 'immediate' && !methodId)
      return setError('Escolha a forma de pagamento para continuar.');
    if (step === 1 && paymentMode === 'installments' && !customerId)
      return setError('Selecione um cliente para registrar a venda a prazo.');
    if (step === 1 && paymentMode === 'installments' && !firstDueDate)
      return setError('Informe a primeira data de vencimento.');
    if (step === 1 && Number(upfront || 0) > 0 && !methodId)
      return setError('Escolha a forma de pagamento da entrada.');
    if (step === 1 && Number(discount || 0) > subtotal)
      return setError('O desconto não pode ser maior que o subtotal da venda.');
  ...
  function close() {
    ...
    setStep(0);
    setError(undefined);
    if (editingSale) router.replace('/vendas');
  }
  function resetForm() {
  ...
  async function createOrUpdateSale() {
    ...
    if (!created.ok || !sale.id || !sale.total)
      throw new Error(sale.message ?? 'Não foi possível criar a venda.');
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (paymentMode === 'immediate' && !method) {
      setError('Revise a forma de pagamento.');
      setPending(false);
      return;
    }
    ...
    try {
      ...
      const immediateAmount =
        paymentMode === 'immediate'
          ? Number(sale.total)
          : Math.min(Number(upfront || 0), Number(sale.total));
      const confirmed = await fetch(`/api/sales/${sale.id}/confirm`, {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          installmentPlan:
            ...
            immediateAmount < Number(sale.total)
      ...
      if (!confirmed.ok)
        throw new Error(
          confirmation.message ?? 'Não foi possível confirmar a venda.',
        );
      close();
      resetForm();
      router.refresh();
  ...
  if (!methods.some((method) => method.status === 'active'))
    return (
      <p className="form-error">
        Configure uma forma de pagamento vinculada a uma conta financeira antes
        de registrar uma venda.
      </p>
    );
  if (!open)
    return (
  ...
  return (
    <section className="form-card wizard-card">
      <header className="form-card-heading wizard-heading">
        <div>
          <span className="wizard-eyebrow">Vendas</span>
          <h2>
            {editingSale
              ? `Editar venda #${editingSale.id.slice(0, 8)}`
              : 'Nova venda'}
          </h2>
          <p>
            {editingSale
              ? 'Revise o rascunho e conclua quando os dados estiverem corretos.'
              : 'Registre a operação em poucos passos. Seus dados permanecem salvos ao voltar.'}
          </p>
        </div>
        <button
        ...
        <div className="wizard-layout">
              ...
              <>
                <div className="wizard-stage-heading">
                  ...
                  <div>
                    ...
                    <p>
                      Selecione os produtos e informe as quantidades desta
                      operação.
                    </p>
                  </div>
                </div>
                <fieldset className="sale-lines">
                  <legend className="sr-only">Itens da venda</legend>
                  {lines.map((line, index) => (
                    <div className="sale-line" key={index}>
                      ...
                      <div className="field">
                        <label htmlFor={`sale-quantity-${index}`}>
                          Quantidade
                        </label>
                        <input
            ...
            {step === 1 && (
              <>
                <div className="wizard-stage-heading">
                  <span>2</span>
                  <div>
                    <h3>Pagamento e dados da venda</h3>
                    <p>
                      Escolha como o cliente pagará e complete os dados
                      comerciais.
                    </p>
                  </div>
                </div>
                ...
                {paymentMode === 'installments' && (
                  <div className="wizard-form-section">
                    ...
                    <div className="form-grid wizard-form-grid-three">
                      ...
                      />
                      <div className="field">
                        <label htmlFor="sale-installments">
                          Número de parcelas
                        </label>
                        <input
                ...
                <div className="wizard-form-section">
                  ...
                  <div className="form-grid wizard-form-grid-three">
                    ...
                    <CurrencyInput
                      ...
                      min={0}
                      max={subtotal}
                      hint="Registrado separadamente do preço."
                    />
                  </div>
                </div>
              </>
              ...
              <>
                <div className="wizard-stage-heading">
                  ...
                  <div>
                    ...
                    <p>
                      Confira os dados antes de concluir. Estoque e financeiro
                      serão atualizados.
                    </p>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <dd>
                      {paymentMode === 'immediate'
                        ...
                        : `${installmentCount}x a prazo${Number(upfront || 0) > 0 ? ' com entrada' : ''}`}
                    </dd>
                  </div>
                  <div>
                    <dt>Estoque</dt>
                    <dd>Será baixado automaticamente</dd>
                  </div>
                  <div>
                    <dt>Financeiro</dt>
                    <dd>
                      {paymentMode === 'immediate'
                        ? 'Recebimento e caixa serão atualizados'
                        : 'Parcelas serão criadas automaticamente'}
                    </dd>
                  </div>
                </section>
                <p className="wizard-confirmation-note">
                  Ao confirmar, esta venda passa a compor o histórico
                  operacional e não será apagada.
                </p>
              </>
            )}
          </div>
          <aside className="wizard-side-summary">
            ...
            <dl>
              ...
              <div>
                <dt>Desconto</dt>
                <dd>{money(Number(discount || 0))}</dd>
              </div>
              {paymentMode === 'installments' && (
                <div>
                  <dt>Entrada</dt>
                  <dd>{money(Number(upfront || 0))}</dd>
                </div>
              )}
            </dl>
            ...
            <p>
              {paymentMode === 'installments'
                ? `Saldo parcelado: ${money(Math.max(0, estimatedTotal - Number(upfront || 0)))}`
                : 'Os valores podem ser revisados até a confirmação.'}
            </p>
          </aside>
        </div>
        {error && (
