...
export function QuickPurchaseForm({ products, suppliers, initialOpen = false, editingPurchase }: { products: Product[]; suppliers: Supplier[]; initialOpen?: boolean; editingPurchase?: EditablePurchase }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      close(); setLines([{ productId: "", quantity: "1", unitCost: "" }]); setSupplierId(""); setFreight(""); setHasFreight(false); setIdempotencyKey(""); router.refresh();
      ...
      <div className="wizard-layout">
        <div className="wizard-main">
          ...
          {step === 2 && <>
            ...
            <section className="summary-list"><div><dt>Itens recebidos</dt><dd>{lines.length} {lines.length === 1 ? "produto" : "produtos"}</dd></div><div><dt>Fornecedor</dt><dd>{suppliers.find(supplier => supplier.id === supplierId)?.name ?? "Não informado"}</dd></div><div><dt>Estoque</dt><dd>Será atualizado automaticamente</dd></div><div><dt>Custo</dt><dd>{hasFreight ? "Será calculado com o rateio do frete" : "Será calculado pelos itens recebidos"}</dd></div></section>
            <p className="wizard-confirmation-note">Ao confirmar, a movimentação de estoque será registrada e mantida no histórico.</p>
          </>}
        </div>
        <aside className="wizard-side-summary"><div className="wizard-summary-heading"><span>Resumo</span><h3>Resumo da compra</h3></div><dl><div><dt>Itens</dt><dd>{lines.length}</dd></div><div><dt>Unidades</dt><dd>{lines.reduce((sum, line) => sum + Number(line.quantity || 0), 0)}</dd></div><div><dt>Fornecedor</dt><dd>{suppliers.find(supplier => supplier.id === supplierId)?.name ?? "A definir"}</dd></div><div><dt>Frete</dt><dd>{money(Number(freight || 0))}</dd></div></dl><div className="wizard-summary"><span>Total estimado</span><strong>{money(total)}</strong></div><p>O custo final será calculado ao confirmar o recebimento.</p></aside>
      </div>
