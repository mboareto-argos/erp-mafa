...
export function StockAdjustmentForm({ products }: { products: Product[] }) {
  ...
  // Estável durante o ciclo de vida do componente: um reenvio após falha (ex.:
  // timeout) usa a mesma chave, permitindo que o backend trate como replay em
  // vez de aplicar o ajuste duas vezes.
  const [idempotencyKey] = useState(() => crypto.randomUUID());
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const values = new FormData(event.currentTarget); setPending(true); setError(undefined);
    try { const response = await fetch("/api/inventory/adjustments", { method: "POST", headers: { "Content-Type": "application/json", "Idempotency-Key": idempotencyKey }, body: JSON.stringify({ productVariantId: values.get("variant"), quantity: Number(values.get("quantity")), reason: values.get("reason") }) }); const data = await response.json() as { message?: string }; if (!response.ok) throw new Error(data.message || "Não foi possível registrar o ajuste."); setOpen(false); router.refresh(); } catch (cause) { setError(cause instanceof Error ? cause.message : "Não foi possível registrar o ajuste."); } finally { setPending(false); }
  }
