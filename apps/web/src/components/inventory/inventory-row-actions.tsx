...
const movementLabel: Record<InventoryMovement["type"], string> = { in: "Entrada", out: "Saída", adjustment: "Ajuste", return: "Devolução", reservation: "Reserva", release: "Liberação de reserva" };
...
function ReserveStock({ row, customers, open, onOpenChange }: { row: InventoryRow; customers: Customer[]; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  async function reserve() {
    ...
    if (!quantityValue || quantityValue <= 0) return setError("Informe uma quantidade válida.");
    if (quantityValue > available) return setError("Quantidade maior que o disponível.");
    setPending(true);
    try {
      const created = await fetch("/api/sales", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ customerId, channel: "presencial", items: [{ productVariantId: row.productVariantId, quantity: quantityValue, unitPrice: Number(unitPrice || 0) }] }) });
      const sale = await created.json() as { id?: string; message?: string };
      if (!created.ok || !sale.id) throw new Error(sale.message ?? "Não foi possível criar a venda para reserva.");
      const reserved = await fetch(`/api/sales/${sale.id}/reserve`, { method: "POST", headers: { "Idempotency-Key": crypto.randomUUID() } });
      const result = await reserved.json() as { message?: string };
      if (!reserved.ok) throw new Error(result.message ?? "Não foi possível reservar o estoque.");
      onOpenChange(false); router.refresh();
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Não foi possível reservar o estoque.");
    } finally {
      setPending(false);
    }
  }

  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><div className="dialog-heading"><div><DialogTitle>Reservar estoque</DialogTitle><DialogDescription>Separa {row.productName} para um cliente sem baixar do físico. Cria uma venda em rascunho já reservada.</DialogDescription></div><DialogClose className="dialog-close" aria-label="Fechar">×</DialogClose></div>
    <div className="dialog-form"><SelectField label="Cliente" value={customerId} onChange={event => setCustomerId(event.target.value)}><option value="">Selecione um cliente</option>{activeCustomers.map(customer => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</SelectField>
      <div className="form-grid wizard-form-grid-three"><div className="field"><label htmlFor="reserve-quantity">Quantidade</label><input id="reserve-quantity" type="number" min="0.001" max={available} step="0.001" value={qty} onChange={event => setQty(event.target.value)} /></div><CurrencyInput label="Preço unitário" value={unitPrice} onValueChange={setUnitPrice} min={0} /></div>
      <p className="table-detail">Disponível: {quantity(row.quantityAvailable)} {row.unit}</p>
    </div>
    {error && <p className="form-error" role="alert">{error}</p>}
    <div className="dialog-actions"><DialogClose asChild><button className="button button-secondary" type="button">Voltar</button></DialogClose><button className="button button-primary" type="button" onClick={reserve} disabled={pending}>{pending ? "Reservando…" : "Reservar"}</button></div>
  </DialogContent></Dialog>;
}

export function InventoryRowActions({ row, movements, canAdjust, canReserve, customers }: { row: InventoryRow; movements: InventoryMovement[]; canAdjust: boolean; canReserve: boolean; customers: Customer[] }) {
  const [detailsOpen, setDetailsOpen] = useState(false);
  const [reserveOpen, setReserveOpen] = useState(false);
  const hasAvailable = Number(row.quantityAvailable) > 0;
  return <><DropdownMenu><DropdownMenuTrigger asChild><button className="row-menu-trigger" type="button" aria-label={`Ações de estoque para ${row.productName}`}><AppIcon name="more" /></button></DropdownMenuTrigger><DropdownMenuContent align="end" sideOffset={6}><DropdownMenuItem className="dropdown-item view" onSelect={() => setDetailsOpen(true)}><span><AppIcon name="eye" /></span><div><strong>Visualizar histórico</strong><small>Entender a formação do saldo</small></div></DropdownMenuItem>{canReserve && hasAvailable && <DropdownMenuItem className="dropdown-item success" onSelect={() => setReserveOpen(true)}><span><AppIcon name="inventory" /></span><div><strong>Reservar</strong><small>Separar para um cliente</small></div></DropdownMenuItem>}{canAdjust && <DropdownMenuItem asChild><Link className="dropdown-item edit" href={`/estoque?new=adjustment&variant=${row.productVariantId}`}><span><AppIcon name="edit" /></span><div><strong>Ajustar estoque</strong><small>Registrar entrada ou saída manual</small></div></Link></DropdownMenuItem>}</DropdownMenuContent></DropdownMenu><InventoryDetails row={row} movements={movements} open={detailsOpen} onOpenChange={setDetailsOpen} />{canReserve && <ReserveStock row={row} customers={customers} open={reserveOpen} onOpenChange={setReserveOpen} />}</>;
}
