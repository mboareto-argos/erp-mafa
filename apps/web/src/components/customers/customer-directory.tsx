...
type CustomerDetail = CustomerListItem & {
  ...
  recentSales: Array<{
    id: string;
    status:
      'draft' | 'confirmed' | 'cancelled' | 'partially_returned' | 'returned';
    total: string;
    createdAt: string;
    _count: { items: number };
  }>;
...
const saleStatus: Record<
  CustomerDetail['recentSales'][number]['status'],
  string
> = {
  draft: 'Rascunho',
...
function DuplicateNotice({ message }: { message: string }) {
  return (
    <div className="customer-duplicate-notice" role="alert">
      <AppIcon name="customers" />
      <div>
        <strong>Possível cliente duplicado</strong>
        <p>{message} Você pode revisar os dados ou salvar mesmo assim.</p>
      </div>
    </div>
  );
}
...
function CustomerForm({
  ...
  function findDuplicate(formData: FormData) {
    ...
    const phones = [formData.get('whatsapp'), formData.get('phone')]
      .map((value) => value?.toString().replace(/\D/g, ''))
      .filter(Boolean);
    const duplicate = customers.find((candidate) => {
      if (candidate.id === customer?.id) return false;
      const candidatePhones = [candidate.whatsapp, candidate.phone]
        .map((value) => value?.replace(/\D/g, ''))
        .filter(Boolean);
      return (
        Boolean(email && candidate.email?.trim().toLowerCase() === email) ||
        phones.some((phone) => candidatePhones.includes(phone))
      );
    });
    return duplicate
      ? `Já existe um cadastro chamado “${duplicate.name}” com o mesmo e-mail ou telefone.`
      : undefined;
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(
          result.message ??
            `Não foi possível ${customer ? 'editar' : 'cadastrar'} o cliente.`,
        );
      onClose();
  ...
  return (
    <section className="form-card customer-workspace-form">
      <header className="form-card-heading">
        <div>
          <span className="wizard-eyebrow">Relacionamento</span>
          <h2>
            {customer ? `Editar cliente · ${customer.name}` : 'Novo cliente'}
          </h2>
          <p>
            Organize os dados essenciais para identificar e atender o cliente
            com agilidade.
          </p>
        </div>
        <button
      ...
      >
        <section className="wizard-form-section">
          ...
          <div className="form-grid">
            ...
            <div className="field">
              <label htmlFor="customer-birth-date">
        ...
        <div className="customer-privacy-note">
          <AppIcon name="shield" />
          <p>
            Use apenas dados necessários para o relacionamento comercial e
            respeite o consentimento do cliente.
          </p>
        </div>
...
function CustomerDetails({
  ...
  useEffect(() => {
    ...
    fetch(`/api/customers/${customer.id}`)
      .then(async (response) => {
        const result = (await response.json()) as CustomerDetail & {
          message?: string;
        };
        if (!response.ok)
          throw new Error(
            result.message ?? 'Não foi possível carregar a ficha do cliente.',
          );
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sale-detail-dialog customer-detail-dialog">
        <header className="sale-detail-header">
          <div className="sale-detail-title">
            ...
            <div>
              <span className="sale-detail-eyebrow">Ficha do cliente</span>
              <DialogTitle>{customer.name}</DialogTitle>
              <DialogDescription>
                {customer.email ||
                  customer.whatsapp ||
                  customer.phone ||
                  'Cliente sem contato informado'}
              </DialogDescription>
            </div>
          </div>
        ...
        <div className="sale-detail-layout">
          <div className="sale-detail-main">
            <section className="sale-detail-card customer-contact-summary">
              <div>
                <span>WhatsApp</span>
              ...
              <div>
                <span>Telefone</span>
                <strong>{customer.phone || 'Não informado'}</strong>
              </div>
              <div>
                <span>E-mail</span>
              ...
              <div>
                <span>Instagram</span>
                <strong>{customer.instagram || 'Não informado'}</strong>
              </div>
            </section>
              ...
              <div className="sale-detail-section-heading">
                <div>
                  ...
                  <div>
                    ...
                    <p>Últimas vendas vinculadas a este cadastro.</p>
                  </div>
                </div>
                {detail && <strong>{detail.recentSales.length}</strong>}
              </div>
              {loading ? (
                <div className="sale-detail-empty">
                  <span>…</span>
                  <p>Carregando relacionamento do cliente…</p>
                </div>
              ) : error ? (
                <div className="sale-detail-empty">
                  <span>!</span>
                    ...
                    <tbody>
                      {detail.recentSales.map((sale) => (
                        <tr key={sale.id}>
                          ...
                          <td data-label="Data">{dateTime(sale.createdAt)}</td>
                ...
                <div className="sale-detail-empty">
                  <span>—</span>
                  <p>Nenhuma venda vinculada a este cliente.</p>
                </div>
              )}
            </section>
            <section className="sale-detail-card customer-data-care">
              <AppIcon name="shield" />
              <div>
                <h3>Proteção de dados</h3>
                <p>
                  Dados pessoais devem ser corrigidos ou removidos conforme a
                  política aplicável, sem apagar o histórico operacional
                  obrigatório.
                </p>
              </div>
            </section>
          ...
          <aside className="sale-financial-summary">
            <div className="sale-financial-heading">
              ...
              <p>Indicadores calculados a partir das operações vinculadas.</p>
            </div>
            <dl>
              <div>
              ...
              <div>
                <dt>Ticket médio</dt>
                <dd>{summary ? money(summary.averageTicket) : '—'}</dd>
              </div>
              <div>
                <dt>Produtos diferentes</dt>
                <dd>{summary?.productsPurchased ?? '—'}</dd>
              </div>
              <div>
                <dt>Última compra</dt>
                <dd>
            ...
            <div className="sale-detail-state draft">
              <AppIcon name="shield" />
              <p>
                Os indicadores consideram somente registros da empresa atual e
                preservam o histórico após a inativação.
              </p>
            </div>
          </aside>
        </div>
  ...
  async function changeStatus() {
    ...
    try {
      const response = await fetch(`/api/customers/${customer.id}/status`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: customer.status === 'active' ? 'deactivate' : 'activate',
        }),
      });
      const result = (await response.json()) as { message?: string };
      if (!response.ok)
        throw new Error(
          result.message ?? 'Não foi possível atualizar o status do cliente.',
        );
      setStatusOpen(false);
      router.refresh();
    } catch (caught) {
    ...
    <>
      <tr>
        <td data-label="Cliente">
          ...
          {customer.instagram && (
            <small className="table-detail">{customer.instagram}</small>
          )}
        </td>
        <td data-label="Contato">
          {customer.whatsapp || customer.phone || 'Não informado'}
        </td>
        <td data-label="E-mail">{customer.email || 'Não informado'}</td>
        <td data-label="Nascimento">
          {customer.birthDate ? civilDate(customer.birthDate) : '—'}
        </td>
        <td data-label="Status">
          <span className={`status-badge ${customer.status}`}>
            {customer.status === 'active' ? 'Ativo' : 'Inativo'}
            ...
            <DropdownMenuContent align="end" sideOffset={6}>
              ...
              >
                ...
                <div>
                  ...
                  <small>Ver ficha e relacionamento</small>
                </div>
              </DropdownMenuItem>
              {canManage && (
                <DropdownMenuItem
                ...
                >
                  ...
                  <div>
                    ...
                    <small>Atualizar dados de contato</small>
                  </div>
                </DropdownMenuItem>
              )}
              {canManage && (
                <DropdownMenuItem
                  className={`dropdown-item ${customer.status === 'active' ? 'danger' : 'success'}`}
                  onSelect={() => setStatusOpen(true)}
                >
                  <span>
                    <AppIcon
                      name={customer.status === 'active' ? 'cancel' : 'shield'}
                    />
                  </span>
                  <div>
                    <strong>
              ...
              <DialogContent>
                <div className="dialog-heading">
                  <div>
                    ...
                    <DialogDescription>
                      {customer.status === 'active'
                        ? 'O cliente deixará de aparecer em novas vendas, mas seu histórico será preservado.'
                        : 'O cliente voltará a ficar disponível para novas vendas.'}
                    </DialogDescription>
                  </div>
                  <DialogClose className="dialog-close" aria-label="Fechar">
                    ×
                ...
                <div className="dialog-actions">
                  ...
                  <button
                    className={`button ${customer.status === 'active' ? 'button-danger' : 'button-primary compact-button'}`}
                    type="button"
                    disabled={pending}
...
export function CustomerDirectory({
      ...
      <tr className="table-empty-row">
        <td className="table-empty-cell" colSpan={6}>
          <div className="table-empty-content">
            ...
            <p>
              {query
                ? 'Tente outro termo ou limpe a busca para visualizar todos os clientes.'
                : 'Cadastre o primeiro cliente para começar a construir o histórico de relacionamento.'}
            </p>
            {query ? (
              <a
  ...
  return (
    <section className="contact-directory customer-directory">
      {canManage && formOpen && (
        <CustomerForm
          customer={editing}
          customers={customers}
