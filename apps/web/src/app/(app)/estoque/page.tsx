...
export default async function InventoryPage({ searchParams }: { searchParams: Promise<{ new?: string; variant?: string; q?: string; status?: string; page?: string }> }) {
  ...
  const { new: newAction, variant: initialVariantId, q, status, page } = await searchParams;
  ...
  return <main className="page-content"><div className="page-heading"><div><h1>Estoque</h1><p>Consulte saldos e movimentações. Toda correção gera um registro auditável.</p></div></div>
    {canAdjust && products.some(product => product.variants.length > 0) && <div className="page-workspace-action"><StockAdjustmentForm products={products} balances={balances} initialOpen={formOpen} initialVariantId={initialVariantId} /></div>}
