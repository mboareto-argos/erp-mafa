...
import {
  ...
  type InventoryCount,
} from '@/components/inventory/inventory-count-workspace';
...
type Balance = {
  ...
  productVariant: { id: string; product: { name: string; sku: string } };
};
...
export default async function InventoryPage({
  ...
  const [balances, lowStock, products, movements, session, counts, customers] =
    await Promise.all([
      backendAuthenticatedRequest('/inventory/balances') as Promise<Balance[]>,
      backendAuthenticatedRequest('/inventory/low-stock') as Promise<
        LowStock[]
      >,
      backendAuthenticatedRequest('/catalog/products') as Promise<Product[]>,
      backendAuthenticatedRequest('/inventory/movements') as Promise<
        InventoryMovement[]
      >,
      getSession() as Promise<{ permissions: string[] } | null>,
      backendAuthenticatedRequest('/inventory/counts') as Promise<
        InventoryCount[]
      >,
      backendAuthenticatedRequest('/customers') as Promise<Customer[]>,
    ]);
  const canAdjust = session?.permissions.includes('adjust_stock') ?? false;
  const canReserve = session?.permissions.includes('manage_sales') ?? false;
  const {
    new: newAction,
      ...
      const stockState: InventoryState =
        Number(available) <= 0
          ...
          : lowStockIds.has(product.id)
            ...
            : 'healthy';
      return {
        id: balance?.id ?? `empty-${variant.id}`,
        ...
        quantityInTransit: balance?.quantityInTransit ?? '0',
        stockState,
        currentPrice: product.prices[0]?.salePrice ?? '0',
      };
    }),
  );
  ...
  const filteredRows = allRows.filter(
    (row) =>
      (!normalizedQuery ||
        row.productName.toLocaleLowerCase('pt-BR').includes(normalizedQuery) ||
        row.sku.toLocaleLowerCase('pt-BR').includes(normalizedQuery)) &&
      (!status || status === 'all' || row.stockState === status),
  );
  const pageSize = 20;
  ...
  movements.forEach((movement) =>
    movementsByVariant.set(movement.productVariantId, [
      ...(movementsByVariant.get(movement.productVariantId) ?? []),
      movement,
    ]),
  );
    ...
    <main className="page-content">
      <div className="page-heading">
        <div>
          <h1>Estoque</h1>
          <p>
            Consulte saldos e movimentações. Toda correção gera um registro
            auditável.
          </p>
        </div>
      </div>
      {activeCount ? (
        ...
        <>
          <div className="page-workspace-action inventory-page-actions">
            {canAdjust &&
            ...
            <InventoryCountWorkspace counts={counts} canAdjust={canAdjust} />
          </div>
          <ListingMetrics
            metrics={[
              {
                label: 'Variações controladas',
                value: allRows.length,
                detail: 'Com posição individual de estoque',
                icon: 'inventory',
              },
              {
                label: 'Produtos em estoque baixo',
                value: lowStock.length,
                detail: 'Abaixo ou igual ao mínimo',
                icon: 'inventory',
              },
              {
                label: 'Sem estoque',
                value: allRows.filter((row) => row.stockState === 'zero')
                  .length,
                detail: 'Variações com saldo zerado',
                icon: 'products',
              },
            ]}
          />
          {lowStock.length > 0 && (
            <section
              className="alert-card inventory-alert"
            ...
            >
              ...
              <div>
                ...
                <span>
                  O saldo disponível está igual ou abaixo do estoque mínimo
                  definido no catálogo.
                </span>
              </div>
            </section>
          )}
          <form method="GET" className="listing-search inventory-filters">
            <div className="field">
              <label htmlFor="inventory-search">Buscar produto ou SKU</label>
              <input
                id="inventory-search"
                name="q"
                defaultValue={q}
                placeholder="Ex.: perfume ou SKU-001"
              />
            </div>
            <SelectField
              label="Situação"
            ...
            >
              ...
              <option value="healthy">Regular</option>
          ...
          <ListingTable
            headers={
              <>
                ...
                <th>SKU</th>
                <th>Situação</th>
                <th className="number">Disponível</th>
                <th className="number">Reservado</th>
                <th className="number">Em trânsito</th>
                  ...
                  <div className="table-empty-content">
                    ...
                    <strong>
                      {q || (status && status !== 'all')
                        ...
                        : 'Nenhum produto disponível para controle'}
                    </strong>
                    <p>
                      {q || (status && status !== 'all')
                        ? 'Altere os filtros ou limpe a busca para visualizar outras posições de estoque.'
                        : 'Cadastre um produto para começar. O saldo será formado por compras, vendas e ajustes autorizados.'}
                    </p>
                    {q || (status && status !== 'all') ? (
                      <a
                        className="button button-secondary compact-button"
                ...
                <tr key={row.id}>
                  <td data-label="Produto">
                    ...
                    {row.productStatus === 'inactive' && (
                      <small className="table-detail">Produto inativo</small>
                    )}
                  </td>
                  <td data-label="SKU">{row.sku}</td>
                  <td data-label="Situação">
                    <span className={`inventory-state-badge ${row.stockState}`}>
                      {stateLabel[row.stockState]}
                    </span>
                  </td>
                  <td className="number" data-label="Disponível">
                    <strong>{quantity(row.quantityAvailable)}</strong>{' '}
                    {row.unit}
                  </td>
                  <td className="number" data-label="Reservado">
                    {quantity(row.quantityReserved)} {row.unit}
                  </td>
                  <td className="number" data-label="Em trânsito">
                  ...
                  <td className="table-actions-cell" data-label="Ações">
                    <InventoryRowActions
                      row={row}
                      movements={
                        movementsByVariant.get(row.productVariantId) ?? []
                      }
                      canAdjust={canAdjust}
          ...
          <ListingPagination
            ...
            extraParams={{ status }}
          />
          <section className="content-section inventory-movements">
            <div className="content-section-heading">
              <div>
                <h2>Movimentações recentes</h2>
                <p>
                  Entradas, saídas, devoluções e ajustes que alteraram os
                  saldos.
                </p>
              </div>
            </div>
              ...
              {movements.length === 0 ? (
                <tr className="table-empty-row">
                  <td className="table-empty-cell" colSpan={6}>
                    <div className="table-empty-content">
                      ...
                      <p>
                        Recebimentos de compras, vendas e ajustes aparecerão
                        automaticamente neste histórico.
                      </p>
                    </div>
                  </td>
                </tr>
                ...
                movements.slice(0, 20).map((movement) => (
                  <tr key={movement.id}>
                    <td data-label="Produto">
                      <strong>
                        {movement.productVariant?.product.name ?? 'Produto'}
                      </strong>
                      <small className="table-detail">
                        {movement.productVariant?.skuVariant ??
                          movement.productVariant?.product.sku}
                      </small>
                    </td>
                    <td data-label="Tipo">
                      <span
                    ...
                    <td data-label="Data">{dateTime(movement.createdAt)}</td>
                  </tr>
                ))
              )}
            </ListingTable>
