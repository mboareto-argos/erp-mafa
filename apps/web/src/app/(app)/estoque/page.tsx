...
type Product = { id: string; name: string; sku: string; unit: string; minStock: string | null; status: "active" | "inactive"; variants: Array<{ id: string; skuVariant: string | null }>; prices: Array<{ salePrice: string }> };
...
type InventoryRow = { id: string; productId: string; productVariantId: string; productName: string; sku: string; unit: string; productStatus: Product["status"]; quantityAvailable: string; quantityReserved: string; quantityInTransit: string; stockState: InventoryState; currentPrice: string };
...
export default async function InventoryPage({ searchParams }: { searchParams: Promise<{ new?: string; variant?: string; q?: string; status?: string; page?: string; inventory?: string }> }) {
  const [balances, lowStock, products, movements, session, counts, customers] = await Promise.all([
