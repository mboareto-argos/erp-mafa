...
} from '@/components/purchasing/purchase-actions';
...
export default async function PurchasesPage({
  ...
  const [purchases, products, suppliers, session] = await Promise.all([
    backendAuthenticatedRequest('/purchasing/purchases') as Promise<
      PurchaseListItem[]
    >,
  ...
  const editingPurchase = canManage
    ? purchases.find(
        (purchase) => purchase.id === editId && purchase.status === 'draft',
      )
    : undefined;
  const received = purchases.filter(
    (purchase) => purchase.status === 'received',
  );
  const receivedItems = purchases.reduce(
    (total, purchase) =>
      total +
      purchase.items.filter((item) => Number(item.quantityReceived) > 0).length,
    0,
  );
  ...
  return (
    <main className="page-content">
      <div className="page-heading">
        <div>
          <h1>Compras</h1>
          <p>Registre o recebimento para atualizar estoque e custo.</p>
        </div>
      </div>
      {canManage && products.length > 0 && (
        <div className="page-workspace-action">
          <QuickPurchaseForm
            ...
            products={products}
            suppliers={suppliers}
            initialOpen={newAction === 'purchase' || Boolean(editingPurchase)}
            editingPurchase={
              editingPurchase
                ? {
        ...
        metrics={[
          {
            ...
            detail: 'No histórico disponível',
            icon: 'purchases',
          },
          {
            label: 'Recebidas',
            value: received.length,
            detail: 'Com estoque atualizado',
            icon: 'inventory',
          },
          {
            label: 'Itens recebidos',
            value: receivedItems,
            detail: 'Em todas as compras listadas',
            icon: 'products',
          },
        ]}
      />
                ...
                <tr className="table-empty-row">
                  <td className="table-empty-cell" colSpan={6}>
                    <div className="table-empty-content">
                      ...
                      <p>
                        {products.length === 0
                          ? 'O produto é a referência que mantém compras, estoque e vendas conectados.'
                          : 'Quando uma compra for criada, ela aparecerá nesta listagem.'}
                      </p>
                    </div>
                  </td>
                </tr>
                ...
                purchases.map((purchase) => (
                  <tr key={purchase.id}>
                    <td data-label="Compra">
                      <strong>Compra #{purchase.id.slice(0, 8)}</strong>
                    ...
                    <td className="number" data-label="Total">
                      {money(purchaseTotal(purchase), purchase.currency)}
                    </td>
