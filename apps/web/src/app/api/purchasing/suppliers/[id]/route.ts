...
export async function GET(_request: Request, context: RouteContext<"/api/purchasing/suppliers/[id]">) {
  const { id } = await context.params;
  try {
    return NextResponse.json(await backendAuthenticatedRequest(`/purchasing/suppliers/${id}`));
  } catch (error) {
    const status = error instanceof ApiRequestError ? error.status : 502;
    return NextResponse.json({ message: error instanceof Error ? error.message : "Não foi possível carregar o fornecedor." }, { status });
  }
}
