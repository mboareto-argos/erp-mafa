...
export async function GET(_request: Request, context: InvitationContext) {
  try {
    return NextResponse.json(
      await backendRequest(`/invitations/${(await context.params).token}`),
    );
  } catch (error) {
    return NextResponse.json(
      { message: error instanceof Error ? error.message : 'Convite inválido.' },
  ...
  try {
    return NextResponse.json(
      await backendRequest(
        `/invitations/${(await context.params).token}/accept`,
